...
const apiKey =
  ...
  'AIzaSyBAF5YLdwYEkia0NeK_Od7pecKNe8RXXsc';
const ai = apiKey ? new GoogleGenAI({ apiKey }) : null;

export async function generateAIGuidance(
  wheel: LifeArea[]
): Promise<{ insight: string; tasks: SMARTTask[] }> {
  if (!ai) {
    return {
      insight:
        'Configure sua API key do Gemini para receber insights personalizados.',
      tasks: [],
    };
  }
...
export async function generateEvolutionInsight(
  oldWheel: LifeArea[],
  newWheel: LifeArea[]
): Promise<string> {
  if (!ai) {
...
export async function simplifyTasks(
  currentTasks: SMARTTask[]
): Promise<SMARTTask[]> {
  if (!ai) {
    return currentTasks;
  }
